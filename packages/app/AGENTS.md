...
- `clawc dev web` proxies `https://app.clawc.ai`, so local UI/CSS changes will not show there.
- For local UI changes, run the backend and app dev servers separately.
- Backend (from `packages/clawc`): `bun run --conditions=browser ./src/index.ts serve --port 4096`
