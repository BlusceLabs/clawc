...
import { type Session } from "@clawc/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("clawc://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("clawc://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("clawc://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("clawc://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("clawc://open-project")).toBeUndefined()
    expect(parseDeepLink("clawc://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "clawc://open-project?directory=/a",
      "clawc://other?directory=/b",
      "clawc://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("clawc://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("clawc://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("clawc://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("clawc://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "clawc://new-session?directory=/a",
      "clawc://open-project?directory=/b",
      "clawc://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __CLAWC__: {
        deepLinks: ["clawc://open-project?directory=/a"],
      },
    } as unknown as Window & { __CLAWC__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["clawc://open-project?directory=/a"])
