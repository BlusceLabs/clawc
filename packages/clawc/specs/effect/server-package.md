...
Practical reference for a future `packages/server` split after the clawc
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/clawc`.
...
Until enough shared service code lives outside `packages/clawc`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/clawc`

It should not import `packages/clawc` services while `packages/clawc`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on clawc-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/clawc` implementation details.
