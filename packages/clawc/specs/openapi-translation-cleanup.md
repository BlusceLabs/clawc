...
Trim `packages/clawc/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/clawc/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/clawc`.
- `[x]` `bun typecheck` from `packages/clawc`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/clawc/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/clawc`.
- `[x]` `bun dev generate > /tmp/clawc-openapi.json` from `packages/clawc`.
...
- Edit `packages/clawc/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/clawc-openapi.json` from `packages/clawc`.
...
- Check source schemas in files like `packages/clawc/src/session/schema.ts`, `packages/clawc/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/clawc/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/clawc/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
