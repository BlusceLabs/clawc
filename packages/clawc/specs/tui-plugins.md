...
- Author package entrypoint is `@clawc/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/clawc-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@clawc/plugin/tui`.
- `@clawc/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@clawc/plugin/tui"
...
- If `engines.clawc` is absent, no check is performed (backward compatible).
...
- `clawc plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `clawc plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.clawc` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.clawc`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.clawc`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.clawc` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@clawc/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@clawc/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@clawc/plugin/tui`.
...
- `message` is required; `title` defaults to `"clawc"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `clawc.default`.
...
- Local plugins persist installed themes under the local `.clawc/themes` area near the plugin config source.
...
- Local smoke plugin: `.clawc/plugins/tui-smoke.tsx`
- Local vim plugin: `.clawc/plugins/tui-vim.tsx`
- Local smoke config: `.clawc/tui.json`
- Local smoke theme: `.clawc/plugins/smoke-theme.json`
