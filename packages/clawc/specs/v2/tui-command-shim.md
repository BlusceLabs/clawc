...
Delete `packages/clawc/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/clawc/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/clawc/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/clawc`
- TUI plugin loader tests in `packages/clawc` if runtime plugin API wiring changed
