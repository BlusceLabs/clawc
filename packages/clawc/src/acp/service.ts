...
export const AuthMethodID = "clawc-login"
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `clawc auth login` in the terminal",
      name: "Login with clawc",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, clawc provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const clawcProvider = providers[ProviderV2.ID.make("clawc")]
  const clawcModel = clawcProvider ? Provider.sort(Object.values(clawcProvider.models))[0] : undefined
  if (clawcProvider && clawcModel) return { providerID: clawcProvider.id, modelID: clawcModel.id }
