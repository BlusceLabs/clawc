import type { PermissionV1 } from "@clawc/core/v1/permission"
import { FSUtil } from "@clawc/core/fs-util"
// CLI entry point for `clawc run` and `clawc --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`clawc --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`clawc --mini --attach`): connects to a running
//      clawc server and runs interactive mode against it.
//
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@clawc/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run clawc with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running clawc server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to CLAWC_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to CLAWC_SERVER_USERNAME or 'clawc')",
      })
      .option("dir", {
        type: "string",
