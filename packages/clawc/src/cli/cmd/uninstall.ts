...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall clawc and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g varth-ai",
      pnpm: "pnpm uninstall -g varth-ai",
      bun: "bun remove -g varth-ai",
      yarn: "yarn global remove varth-ai",
      brew: "brew uninstall clawc",
      choco: "choco uninstall clawc",
      scoop: "scoop uninstall clawc",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "clawc", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# clawc") || content.includes(".clawc/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".clawc/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".clawc/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".clawc"))
    ) {
      continue
    }
