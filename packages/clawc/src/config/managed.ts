...
const MANAGED_PLIST_DOMAIN = "ai.clawc.managed"
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/clawc"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "clawc")
    default:
      return "/etc/clawc"
  }
}
...
export function managedConfigDir() {
  return process.env.CLAWC_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
