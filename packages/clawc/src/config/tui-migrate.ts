...
const TUI_SCHEMA_URL = "https://clawc.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from clawc.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const clawc = await clawcFiles(input)
  for (const file of clawc) {
...
async function clawcFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "clawc"),
    ...(await Filesystem.findUp(["clawc.json", "clawc.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "clawc"))
  }
  if (Flag.CLAWC_CONFIG) files.push(Flag.CLAWC_CONFIG)
