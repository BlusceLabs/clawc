...
import { isRecord } from "@clawc/tui/util/record"
...
import { TuiKeybind } from "@clawc/tui/config/keybind"
...
export class Service extends Context.Service<Service, Interface>()("@clawc/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old clawc.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.clawc`
  // folders between cwd and home, and CLAWC_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.CLAWC_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit CLAWC_TUI_CONFIG override, if set.
  if (Flag.CLAWC_TUI_CONFIG) {
    const configFile = Flag.CLAWC_TUI_CONFIG
  ...
  // 4. `.clawc` directories (and CLAWC_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".clawc") || dir === Flag.CLAWC_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".clawc") && dir !== Flag.CLAWC_CONFIG_DIR) continue
