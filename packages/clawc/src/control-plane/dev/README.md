This is a plugin to simulate a remote environment locally. Add this to `.clawc/clawc.jsonc`:

```json
  "plugin": ["../packages/clawc/src/control-plane/dev/debug-workspace-plugin.ts"],
```
...
```
./packages/clawc/script/run-workspace-server
```
