...
import { IdeEvent } from "@clawc/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["CLAWC_CALLER"] === "vscode" || process.env["CLAWC_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.clawc"], {
    nothrow: true,
  })
