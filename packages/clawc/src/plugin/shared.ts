...
export const DEPRECATED_PLUGIN_PACKAGES = ["clawc-openai-codex-auth", "clawc-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, clawcVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(clawcVersion) || semver.major(clawcVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.clawc
  if (typeof range !== "string") return
  if (!semver.satisfies(clawcVersion, range)) {
    throw new Error(`Plugin requires clawc ${range} but running ${clawcVersion}`)
  }
}
