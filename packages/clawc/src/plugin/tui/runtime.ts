...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@clawc/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { errorData, errorMessage } from "@clawc/tui/util/error"
...
import { hasTheme, upsertTheme } from "@clawc/tui/context/theme"
import { Global } from "@clawc/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@clawc/core/util/flock"
import { Flag } from "@clawc/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@clawc/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@clawc/core/v1/config/plugin"
import { createCommandShim } from "@clawc/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@clawc/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".clawc"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".clawc", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".clawc", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.CLAWC_PURE ? [] : pluginOrigins
    if (Flag.CLAWC_PURE && pluginOrigins.length) {
    }
