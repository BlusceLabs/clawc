...
import type { LLMClientShape } from "@clawc/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("clawc"))
    return { type: "unsupported", reason: "provider is not openai, clawc, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @clawc/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains clawc-owned. The native runtime only adapts
      // the @clawc/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
