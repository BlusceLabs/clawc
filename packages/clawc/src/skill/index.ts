...
import { FrontmatterError } from "@clawc/core/v1/config/error"
import { ConfigMarkdown } from "@/config/markdown"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Glob } from "@clawc/core/util/glob"
...
const CLAWC_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
const SKILL_PATTERN = "**/SKILL.md"

// Built-in skill that ships with clawc. The model's intuition for what an
// clawc.json should look like is often wrong, and clawc hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch clawc's own config files gives it the
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, CLAWC_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@clawc/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_CLAWC_SKILL_NAME] = {
          name: CUSTOMIZE_CLAWC_SKILL_NAME,
          description: CUSTOMIZE_CLAWC_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_CLAWC_SKILL_BODY,
        }
