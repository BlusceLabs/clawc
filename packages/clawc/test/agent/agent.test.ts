...
it.instance("plan agent denies edits except .clawc/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".clawc/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".clawc", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.CLAWC_TEST_HOME
      process.env.CLAWC_TEST_HOME = test.directory
