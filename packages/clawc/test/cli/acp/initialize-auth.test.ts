...
describe("clawc acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ clawc }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ clawc }))
  ...
  cliIt.live(
    ...
    ({ clawc }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ clawc })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("clawc-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "clawc-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.CLAWC_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ clawc }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("clawc-login")
