...
// terminal sizes. The default clawc tui command is excluded —
// `clawc --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `clawc --help` prints today;
...
// top-level help on `--help` and exits 1; not a real clawc command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("clawc CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ clawc }) =>
      Effect.gen(function* () {
        const topLevel = yield* clawc.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* clawc.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`clawc ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`clawc ${argv.join(" ")} --help`)
        }
