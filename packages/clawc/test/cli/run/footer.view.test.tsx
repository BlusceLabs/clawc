...
import type { QuestionRequest } from "@clawc/sdk/v2"
import { OpencodeKeymapProvider, registerOpencodeKeymap } from "@clawc/tui/keymap"
import {
...
test("direct footer separates a lone context hint from model and command hint", async () => {
  const app = await renderFooter({
    providers: [provider()],
    currentModel: { providerID: "clawc", modelID: "gpt-5" },
...
test("direct model panel renders current model selector", async () => {
  ...
  const [current] = createSignal<RunInput["model"]>({ providerID: "clawc", modelID: "gpt-5" })
