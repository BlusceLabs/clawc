// Subprocess integration tests for `clawc run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `clawc.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `CLAWC_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("clawc run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, clawc }) =>
      Effect.gen(function* () {
        ...
        const result = yield* clawc.run("say hi")
        clawc.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, clawc }) =>
      Effect.gen(function* () {
        ...
        const result = yield* clawc.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, clawc }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* clawc.run("think", { extraArgs: ["--thinking"] })
        clawc.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* clawc.run("think again")
        clawc.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, clawc }) =>
      Effect.gen(function* () {
        ...
        const result = yield* clawc.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, clawc }) =>
      Effect.gen(function* () {
        ...
        const result = yield* clawc.run("say hi", { format: "json" })
        clawc.expectExit(result, 0)

        const events = clawc.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ clawc }) =>
      Effect.gen(function* () {
        const result = yield* clawc.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, clawc }) =>
      Effect.gen(function* () {
        ...
        const result = yield* clawc.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, clawc }) =>
      Effect.gen(function* () {
        ...
        const result = yield* clawc.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, clawc }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* clawc.run("request permission", { permission: { bash: "ask" } })
        clawc.expectExit(denied, 0)
        ...
        const allowed = yield* clawc.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        clawc.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* clawc.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        clawc.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, clawc }) =>
      Effect.gen(function* () {
        ...
        const server = yield* clawc.serve()

        const result = yield* clawc.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, clawc }) =>
      Effect.gen(function* () {
        const result = yield* clawc.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, clawc }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* clawc.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
