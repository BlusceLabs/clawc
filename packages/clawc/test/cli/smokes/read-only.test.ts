...
describe("clawc read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // CLAWC_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ clawc }) =>
      Effect.gen(function* () {
        const r = yield* clawc.spawn(["mcp", "list"])
        clawc.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ clawc }) =>
      Effect.gen(function* () {
        const r = yield* clawc.spawn(["providers", "list"])
        clawc.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ clawc }) =>
      Effect.gen(function* () {
        const r = yield* clawc.spawn(["models"])
        clawc.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ clawc }) =>
      Effect.gen(function* () {
        const r = yield* clawc.spawn(["agent", "list"])
        clawc.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh CLAWC_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ clawc }) =>
      Effect.gen(function* () {
        const r = yield* clawc.spawn(["session", "list"])
        clawc.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ clawc }) =>
      Effect.gen(function* () {
        const r = yield* clawc.spawn(["stats"])
        clawc.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under CLAWC_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ clawc }) =>
      Effect.gen(function* () {
        const r = yield* clawc.spawn(["db", "path"])
        clawc.expectExit(r, 0, "db path")
