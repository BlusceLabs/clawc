...
`,
  ...
  const pure = process.env.CLAWC_PURE
  const meta = process.env.CLAWC_PLUGIN_META_FILE
  process.env.CLAWC_PURE = "1"
  process.env.CLAWC_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.CLAWC_PURE
    } else {
      process.env.CLAWC_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.CLAWC_PLUGIN_META_FILE
    } else {
      process.env.CLAWC_PLUGIN_META_FILE = meta
    }
  }
})
