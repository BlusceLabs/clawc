...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ clawc }) =>
    Effect.gen(function* () {
      const result = yield* clawc.spawn(["--replay-limit", "10"])

      clawc.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ clawc }) =>
    Effect.gen(function* () {
      const result = yield* clawc.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ clawc }) =>
    Effect.gen(function* () {
      const result = yield* clawc.spawn(["--mini", "--port", "4096"])
