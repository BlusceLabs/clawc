...
import { SessionTable } from "@clawc/core/session/sql"
import { EventSequenceTable } from "@clawc/core/event/sql"
...
import { WorkspaceV2 } from "@clawc/core/workspace"
import { WorkspaceTable } from "@clawc/core/control-plane/workspace.sql"
...
const originalEnv = {
  CLAWC_AUTH_CONTENT: process.env.CLAWC_AUTH_CONTENT,
  CLAWC_EXPERIMENTAL_WORKSPACES: process.env.CLAWC_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.CLAWC_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@clawc.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.CLAWC_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=clawc-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.CLAWC_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.CLAWC_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.CLAWC_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=clawc-test")
