// Subprocess test harness for the clawc CLI. Spawns the real binary against
...
//
// Configuration flows through clawc's built-in test affordances:
//   - CLAWC_CONFIG_CONTENT      : provider config inline, no files to find
//   - CLAWC_TEST_HOME           : pins os.homedir() → tmpdir
//   - CLAWC_DISABLE_PROJECT_CONFIG : skip walking up for clawc.json
//   - CLAWC_PURE                : skip external plugin discovery + install
//   - CLAWC_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `clawc.run` is fully wired. The shape supports adding more
// builders (`clawc.serve(opts)`, `clawc.acp(opts)`, `clawc.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of OpencodeCli.
import { test, type TestOptions } from "bun:test"
import { FSUtil } from "@clawc/core/fs-util"
import { AppProcess } from "@clawc/core/process"
...
const clawcRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(clawcRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    CLAWC_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    CLAWC_CONFIG_CONTENT: configJson,
    CLAWC_DISABLE_PROJECT_CONFIG: "1",
    CLAWC_PURE: "1",
    CLAWC_DISABLE_AUTOUPDATE: "1",
    CLAWC_DISABLE_AUTOCOMPACT: "1",
    CLAWC_DISABLE_MODELS_FETCH: "1",
    CLAWC_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `clawc run`. New flags should
...
// `clawc serve` is a long-lived process — it never exits on its own.
...
// `clawc acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `clawc serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `clawc acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("clawc.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          CLAWC_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("clawc.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("clawc.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "clawc server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `clawc serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("clawc.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const clawc: OpencodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, clawc })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `clawc.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "clawc") {
...
// every call site. Use this for any test that needs the clawc CLI fixture.
//
...
// resources (e.g. `clawc.serve`) without an extra `Effect.scoped` wrapper —
