...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-clawc-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-clawc-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://clawc.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-clawc-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-clawc-workspace")
