...
import { SessionProjector } from "@clawc/core/session/projector"
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(() => mkdir(path.join(test.directory, "packages", "clawc"), { recursive: true }))
        ...
        const current = yield* withSession({ title: "current" }).pipe(
          provideInstance(path.join(test.directory, "packages", "clawc")),
        )
    ...
    () =>
      Effect.gen(function* () {
        ...
        const ids = (yield* SessionNs.Service.use((session) =>
          session.list({ directory: path.join(test.directory, "packages", "clawc") }),
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const test = yield* TestInstance
        const dir = path.join(test.directory, "packages", "clawc")
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(() =>
          mkdir(path.join(test.directory, "packages", "clawc", "src", "deep"), { recursive: true }),
        )
        yield* Effect.promise(() => mkdir(path.join(test.directory, "packages", "app"), { recursive: true }))
        ...
        const current = yield* withSession({ title: "current" }).pipe(
          provideInstance(path.join(test.directory, "packages", "clawc", "src")),
        )
        const deeper = yield* withSession({ title: "deeper" }).pipe(
          provideInstance(path.join(test.directory, "packages", "clawc", "src", "deep")),
        )
        const sibling = yield* withSession({ title: "sibling" }).pipe(
          provideInstance(path.join(test.directory, "packages", "app")),
        )
        ...
        if (process.platform === "win32") {
          const windowsPathIDs = (yield* SessionNs.Service.use((session) =>
            session.list({ path: "packages\\clawc\\src" }),
          )).map((session) => session.id)
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* Effect.promise(() =>
          mkdir(path.join(test.directory, "packages", "clawc", "src"), { recursive: true }),
        )
        yield* Effect.promise(() => mkdir(path.join(test.directory, "packages", "app"), { recursive: true }))
        ...
        const pathIDs = (yield* SessionNs.Service.use((session) =>
          session.list({
            directory: path.join(test.directory, "packages", "clawc", "src"),
            path: "packages/clawc/src",
          }),
        )).map((session) => session.id)
        expect(pathIDs).toContain(current.id)
