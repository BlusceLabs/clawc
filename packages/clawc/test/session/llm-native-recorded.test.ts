...
import { SessionV1 } from "@clawc/core/v1/session"
import { FSUtil } from "@clawc/core/fs-util"
import { ModelsDev } from "@clawc/core/models-dev"
import { HttpRecorder } from "@clawc/http-recorder"
import { HttpRecorderInternal } from "@clawc/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@clawc/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@clawc/llm/route"
import { Env } from "@/env"
...
const zenURL = (connection: string) => `https://console.clawc.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.CLAWC_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["clawc", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("CLAWC_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("CLAWC_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["clawc", "native", "oauth", "tool-loop"],
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["clawc", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.CLAWC_RECORD_CONSOLE_TOKEN && process.env.CLAWC_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.clawc,
        name: "OpenCode Zen",
        ...
        model,
        options: {
          apiKey: process.env.CLAWC_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.CLAWC_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["clawc", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("CLAWC_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("CLAWC_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("CLAWC_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set CLAWC_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the clawc LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "clawc.json"),
      JSON.stringify({ $schema: "https://clawc.ai/config.json", ...scenario.config(model) }),
    ),
  )
