...
import { LLMEvent, ToolFailure } from "@clawc/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@clawc/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("clawc"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("clawc") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("clawc") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, clawc, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored clawc auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("clawc") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("clawc"),
