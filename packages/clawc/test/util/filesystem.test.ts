...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "clawc.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "clawc.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "clawc.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "clawc.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["clawc.json", "clawc.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "clawc.json"),
        path.join(project, "clawc.jsonc"),
      ])
    })
  })
