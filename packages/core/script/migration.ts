...
async function generate() {
  const temporary = await fs.mkdtemp(path.join(os.tmpdir(), "clawc-core-migration-"))
...
async function check() {
  const temporary = await fs.mkdtemp(path.join(os.tmpdir(), "clawc-core-migration-check-"))
  const incremental = path.join(temporary, "incremental")
  const full = path.join(temporary, "full")
  try {
    await fs.mkdir(incremental)
