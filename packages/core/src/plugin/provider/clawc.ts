...
import type { IntegrationOAuthMethodRegistration } from "@clawc/plugin/v2/effect/integration"
import { define } from "@clawc/plugin/v2/effect/plugin"
...
const defaultServer = "https://console.clawc.ai"
const clientID = "clawc-cli"
...
export const OpencodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("clawc")
      ...
      const item = catalog.provider.get(ProviderV2.ID.clawc)
      if (!item) return
      const hasKey = Boolean(process.env.CLAWC_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("clawc")),
