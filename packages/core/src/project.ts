...
export interface Interface {
   ...
   *
   * This exists while the old clawc project service and this core project
...
export class Service extends Context.Service<Service, Interface>()("@clawc/ProjectV2") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const cached = Effect.fnUntraced(function* (dir: string) {
      return yield* fs.readFileString(path.join(dir, "clawc")).pipe(
    ...
    const commit = Effect.fn("Project.commit")(function* (input: { store: AbsolutePath; id: ID }) {
      yield* fs.writeFileString(path.join(input.store, "clawc"), input.id).pipe(Effect.ignore)
    })
