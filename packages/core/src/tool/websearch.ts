...
import { ToolFailure } from "@clawc/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@clawc/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.CLAWC_WEBSEARCH_PROVIDER === "exa" || process.env.CLAWC_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.CLAWC_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("CLAWC_EXPERIMENTAL") || truthy("CLAWC_ENABLE_EXA") || truthy("CLAWC_EXPERIMENTAL_EXA"),
    enableParallel: truthy("CLAWC_ENABLE_PARALLEL") || truthy("CLAWC_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `clawc/${InstallationVersion}`,
