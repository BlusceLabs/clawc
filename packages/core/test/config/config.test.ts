...
import { Config } from "@clawc/core/config"
import { ConfigProvider } from "@clawc/core/config/provider"
import { ConfigMigrateV1 } from "@clawc/core/v1/config/migrate"
...
import { Location } from "@clawc/core/location"
import { Policy } from "@clawc/core/policy"
...
describe("Config", () => {
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(
                path.join(tmp.path, "clawc.json"),
                JSON.stringify({ $schema: "middle", providers: { middle: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "clawc.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "clawc.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(path.join(tmp.path, "clawc.json"), "{ invalid"),
              fs.writeFile(path.join(tmp.path, "clawc.jsonc"), JSON.stringify({ providers: { invalid: true } })),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .clawc configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".clawc"), { recursive: true })
            await fs.mkdir(path.join(directory, ".clawc"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "clawc.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "clawc.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "clawc.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "clawc.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "config.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".clawc", "clawc.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".clawc", "clawc.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".clawc")),
              AbsolutePath.make(path.join(directory, ".clawc")),
            ])
