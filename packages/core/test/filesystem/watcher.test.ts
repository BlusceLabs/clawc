...
import { Watcher } from "@clawc/core/filesystem/watcher"
...
const flagsLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    CLAWC_EXPERIMENTAL_FILEWATCHER: "true",
    CLAWC_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
...
function withTmp<A, E, R>(
  ...
  return Effect.acquireRelease(
    Effect.promise(async () => {
      ...
      await $`git config user.email test@clawc.test`.cwd(tmp.path).quiet()
