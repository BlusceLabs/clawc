...
import { InstructionContext } from "@clawc/core/instruction-context"
import { Location } from "@clawc/core/location"
import { AbsolutePath } from "@clawc/core/schema"
import { SystemContext } from "@clawc/core/system-context"
import { SystemContextRegistry } from "@clawc/core/system-context/registry"
...
describe("InstructionContext", () => {
  ...
  it.effect("honors the project instruction opt-out", () =>
    Effect.gen(function* () {
      const previous = process.env.CLAWC_DISABLE_PROJECT_CONFIG
      let scanned = false
      process.env.CLAWC_DISABLE_PROJECT_CONFIG = "1"
      ...
      yield* SystemContextRegistry.Service.pipe(
        ...
        Effect.ensuring(
          Effect.sync(() => {
            if (previous === undefined) delete process.env.CLAWC_DISABLE_PROJECT_CONFIG
            else process.env.CLAWC_DISABLE_PROJECT_CONFIG = previous
          }),
        ),
      )
