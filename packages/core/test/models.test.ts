...
// test/preload.ts pins CLAWC_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.CLAWC_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.CLAWC_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.CLAWC_MODELS_PATH = undefined
  Flag.CLAWC_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.CLAWC_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.CLAWC_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.CLAWC_DISABLE_MODELS_FETCH = false
        }),
