...
import { MoveSession } from "@clawc/core/control-plane/move-session"
import { Database } from "@clawc/core/database/database"
import { FSUtil } from "@clawc/core/fs-util"
import { Git } from "@clawc/core/git"
...
import { SessionStore } from "@clawc/core/session/store"
...
async function initRepo(directory: string) {
  ...
  await $`git config user.email test@clawc.test`.cwd(directory).quiet()
