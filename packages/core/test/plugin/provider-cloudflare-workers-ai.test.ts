...
import { CloudflareWorkersAIPlugin } from "@clawc/core/plugin/provider/cloudflare-workers-ai"
  ...
  it.effect("uses env API key over auth or configured API key and keeps the Cloudflare User-Agent", () =>
    withEnv({ CLOUDFLARE_ACCOUNT_ID: "acct", CLOUDFLARE_API_KEY: "env-key" }, () =>
      Effect.gen(function* () {
        ...
        expect(headers["user-agent"]).toMatch(/^clawc\/.* cloudflare-workers-ai \(.+\) ai-sdk\/openai-compatible\//)
      }),
    ),
  )
