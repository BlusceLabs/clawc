...
import { PermissionV2 } from "@clawc/core/permission"
import { SessionV2 } from "@clawc/core/session"
import { ToolRegistry } from "@clawc/core/tool/registry"
import { WebFetchTool } from "@clawc/core/tool/webfetch"
...
describe("WebFetchTool registration", () => {
  ...
  it.effect("retries Cloudflare challenges with an honest user agent", () =>
    Effect.gen(function* () {
      ...
      expect(requests[1]?.headers["user-agent"]).toBe("clawc")
    }),
  )
