...
export async function spawnWslSidecar(
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export CLAWC_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export CLAWC_CLIENT=desktop",
    `export CLAWC_SERVER_USERNAME=${shellEscape(username)}`,
    `export CLAWC_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(clawc)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
