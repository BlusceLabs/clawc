...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-clawc-directory", "directory"],
    ["x-clawc-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-clawc-directory")
  next.headers.delete("x-clawc-workspace")
  return next
}
...
export function createOpencodeClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  ...
  if (config?.directory) {
    config.headers = {
      ...config.headers,
      "x-clawc-directory": encodeURIComponent(config.directory),
    }
  }
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-clawc-workspace": config.experimental_workspaceID,
    }
  }
