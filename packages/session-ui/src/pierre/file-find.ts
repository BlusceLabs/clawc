...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("clawc-find")
  api.delete("clawc-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("clawc-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("clawc-find", new Highlight(...rest))
    return true
  }
