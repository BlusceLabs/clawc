...
import { createOpencode, type ToolPart } from "@clawc/sdk"
...
console.log("🚀 Starting clawc server...")
const clawc = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await clawc.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new clawc session...")
    const { client, server } = clawc
    ...
    console.log("✅ Created clawc session:", createResult.data.id)
  ...
  console.log("📝 Sending to clawc:", message.text)
