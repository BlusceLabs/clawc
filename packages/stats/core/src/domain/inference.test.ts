...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve clawc route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "clawc")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "clawc")).toBe("openai")
    expect(statProvider("big-pickle", "", "clawc")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "clawc"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep clawc as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "clawc"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "clawc"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep clawc or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "clawc"), country: "US" })).toMatchObject([
