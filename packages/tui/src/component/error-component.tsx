...
export function ErrorComponent(props: { error: Error; reset: () => void; mode?: "dark" | "light" }) {
  ...
  // Safe fallback palette per mode (mirrors theme/assets/clawc.json) since the
      ...
      <box width={contentWidth()} flexGrow={1} flexDirection="column" paddingTop={1} paddingBottom={1} gap={1}>
        ...
        <Show when={showFooter()}>
          <box flexDirection="column" alignItems="center" flexShrink={0}>
            ...
            <text fg={colors.muted}>clawc {InstallationVersion}</text>
          </box>
        </Show>
      </box>
    </box>
...
function buildIssueURL(message: string, stack: string) {
  ...
  const url = new URL("https://github.com/BlusceLabs/clawc/issues/new?template=bug-report.yml")
  url.searchParams.set("title", `TUI crash: ${message}`)
  url.searchParams.set("clawc-version", InstallationVersion)
  url.searchParams.set("os", describeOS())
  url.searchParams.set("terminal", describeTerminal())
  url.searchParams.set(
    "reproduce",
    "Reported automatically from the clawc crash screen. If you can, describe what you were doing when it crashed.",
  )
  ...
  const marker = "\n... (truncated)"
  const head = `The clawc TUI crashed with an unexpected error.\n\n**Error:** ${message}\n\n**Stack trace:**\n`
