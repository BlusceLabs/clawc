...
export const CLAWC_BASE_MODE = "base"
...
export function createClawcModeStack(keymap: OpenTuiKeymap) {
  ...
  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(CLAWC_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(CLAWC_MODE_KEY, stack.at(-1)?.mode ?? CLAWC_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? CLAWC_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(CLAWC_MODE_KEY, undefined)
