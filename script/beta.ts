...
const model = "clawc/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/clawc")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with clawc...`)
  ...
  try {
    await $`clawc run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  clawc failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with clawc")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with clawc...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/clawc`.",
