...
import { createOpencode } from "@clawc/sdk"
...
`)
  ...
  const clawc = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await clawc.client.session.create()
    const result = await clawc.client.session
      .prompt({
