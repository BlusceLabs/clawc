...
const repo = process.env.GH_REPO ?? "BlusceLabs/clawc"
const bot = ["actions-user", "github-actions[bot]", "clawc", "clawc-agent[bot]"]
const team = [
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/clawc packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/clawc/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/clawc/")) areas.add("core")
