# Remove `packages/clawc/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/clawc/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the clawc legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/clawc/src/storage/db.ts` are concentrated in 22 source files:

- `packages/clawc/src/account/repo.ts`
- `packages/clawc/src/cli/cmd/db.ts`
- `packages/clawc/src/cli/cmd/import.ts`
- `packages/clawc/src/cli/cmd/stats.ts`
- `packages/clawc/src/control-plane/workspace.ts`
- `packages/clawc/src/index.ts`
- `packages/clawc/src/node.ts`
- `packages/clawc/src/permission/index.ts`
- `packages/clawc/src/project/project.ts`
- `packages/clawc/src/server/projectors.ts`
- `packages/clawc/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/clawc/src/server/shared/fence.ts`
- `packages/clawc/src/session/message-v2.ts`
- `packages/clawc/src/session/projectors.ts`
- `packages/clawc/src/session/prompt.ts`
- `packages/clawc/src/session/session.ts`
- `packages/clawc/src/session/todo.ts`
- `packages/clawc/src/share/share-next.ts`
- `packages/clawc/src/storage/db.ts`
- `packages/clawc/src/sync/index.ts`
- `packages/clawc/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy clawc database wrapper; `packages/clawc/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the clawc wrapper.
...
Status: Completed. `SyncEvent` and the clawc projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy clawc database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy clawc database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy clawc database wrapper.
...
All migration groups are complete or superseded. `packages/clawc/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No clawc data-migration group remains.

The previous clawc `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate clawc data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/clawc`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/clawc/src`
- `bun typecheck` from `packages/clawc`
- Relevant package tests from `packages/clawc`, not the repo root
