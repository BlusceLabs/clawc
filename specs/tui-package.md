...
`packages/clawc/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/clawc`,
`packages/cli`, or `@clawc/core`.
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/clawc`.
- Do not replace private `packages/clawc` imports with `@clawc/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@clawc/tui`.
...
```text
packages/clawc/src/cli/cmd/tui
```

Its private dependency on `packages/clawc` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/clawc/src/*`.
...
Status: Completed. The private `@clawc/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@clawc/tui`.
...
- The package has no dependency on `clawc`, `@clawc/cli`, or
  `@clawc/core`.
...
primitives now live in `@clawc/tui`. The legacy host consumes explicit
...
and build inputs through `@clawc/tui/runtime`. Movable app, component,
...
`@clawc/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/clawc` initially.
...
canonical in `@clawc/tui`. Configured references resolve through the new
...
`@clawc/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/clawc`.
...
- The package has no imports from `packages/clawc`, `packages/cli`, or
  `@clawc/core`.
...
public `@clawc/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/clawc`.
- Keep the legacy embedded worker and server startup in `packages/clawc`.
- Change those adapters to load config, create transport inputs, and call the
  public `@clawc/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/clawc`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@clawc/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@clawc/core`, or either
  executable package.
...
cd packages/clawc && bun typecheck
...
rg '@clawc/core|packages/clawc|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/clawc/src packages/clawc/test
```
...
- Build the current-platform `packages/clawc` binary.
