...
Both `action` and `resource` use clawc's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.clawc` files is intentionally deferred until `.clawc` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether clawc permits an operation through its managed execution paths.
