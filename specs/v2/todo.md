...
The clawc server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@clawc/llm` tool loop after replacing its
...
- plugins get global "clawc" instance like in that post i showed
- clawc instance has stuff like `clawc.session.prompt()` or
  `clawc.tool.register({...})`
...
- decide whether to preserve deprecated `@clawc/llm` orchestration exports
